...
class EvalLoopOutput(NamedTuple):
    ...
    logit_score: Optional[Union[np.ndarray, Tuple[np.ndarray]]]
...
class Trainer:
        ...
        **kwargs,
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            if isinstance(train_dataloader, DataLoader) and isinstance(train_dataloader.sampler, DistributedSampler):
                ...
                print(f'------------base trainer train_dataloader.sampler, len:{len(train_dataloader)}, dataloader:{train_dataloader}, sampler:{train_dataloader.sampler}')
            elif isinstance(train_dataloader.dataset, IterableDatasetShard):
                train_dataloader.dataset.set_epoch(epoch)
            print(f'------------base trainer train_dataloader.dataset, len:{len(train_dataloader)}, dataloader:{train_dataloader}, dataset:{train_dataloader.dataset}')
            ...
            print(f'------------base trainer epoch_iterator, len:{len(epoch_iterator)}')
            ...
            # )
            print(f'------------base trainer train dataset:{self.train_dataset}')
            if train_dataset_is_sized:
                steps_in_epoch = len(epoch_iterator)
                print(f'------------base trainer steps in epoch=len:{steps_in_epoch}')
            else:
                steps_in_epoch =args.max_steps * args.gradient_accumulation_steps
                print(f'------------base trainer steps in epoch max steps* grad=:{steps_in_epoch}, max steps:{args.max_steps}, grad:{args.gradient_accumulation_steps}')
    ...
    def evaluation_loop(
        ...
        preds_host = None
        labels_host = None
        logit_score_host = None
        # losses/preds/labels on CPU (final containers)
        all_losses = None
        all_preds = None
        all_labels = None
        all_logit_scores = None
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels, logit_score = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            ...
            if logit_score is not None:
                logit_score = self._nested_gather(logit_score)
                logit_score_host = logit_score if logit_score_host is None else nested_concat(logit_score_host, logit_score)
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                if logit_score_host is not None:
                    logit_score = nested_numpify(logit_score_host)
                    all_logit_scores = logit_score if all_logit_scores is None else nested_concat(all_logit_scores, logit_score)

                # Set back to None to begin a new accumulation
                losses_host, preds_host, labels_host, logit_score_host = None, None, None, None
        ...
        if all_logit_scores is not None:
            all_logit_scores = nested_truncate(all_logit_scores, num_samples)

        # Metrics!
        if self.compute_metrics is not None and all_preds is not None and all_labels is not None:
            metrics = self.compute_metrics(EvalPrediction(predictions=all_preds, label_ids=all_labels, logit_score=all_logit_scores))
        else:
            metrics = {}
        ...
        return EvalLoopOutput(predictions=all_preds, label_ids=all_labels, metrics=metrics, num_samples=num_samples, logit_score=all_logit_scores)
    ...
    def prediction_loop(
        ...
        logit_score_host: Union[torch.Tensor, List[torch.Tensor]] = None
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if labels is not None:
                labels_host = labels if labels_host is None else nested_concat(labels_host, labels, padding_index=-100)
            # if logit_score is not None:
            #     logit_score = logit_score if logit_score_host is None else nested_concat(logit_score_host, logit_score)
            self.control = self.callback_handler.on_prediction_step(args, self.state, self.control)
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                if not prediction_loss_only:
                    ...
                    # logit_score.add.add_arrays(self._gather_and_numpify(labels_host, "eval_logit_ids"))
